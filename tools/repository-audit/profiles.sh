...
# Keep this one published exception immutable; environment overrides are ignored.
...
run_commitlint() {
  ...
  if [ -n "$from_ref" ]; then
    ...
    run_commitlint_ranges "$from_ref" "$to_ref" "$commitlint_cmd" || return
  else
